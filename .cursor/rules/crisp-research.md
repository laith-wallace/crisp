...
For each at-risk dimension, be ready to state *why this feature type* risks it in one specific sentence - that sentence goes in the DIMENSIONS AT RISK output. "Form / Input risks R because validation typically fires only on submit, not on blur" - not "R is at risk".
...
- Source access is unreliable in practice: Mobbin requires authentication, and SaaSpo and Screenlane index poorly in general web search. Use whatever search tool is available, fall down the priority order when a source is inaccessible, and skip dead sources silently.
- Patterns may come from model knowledge when search is thin - label every pattern `[verified via search]` or `[from knowledge]`. Never present a knowledge pattern as a searched one.
...
## Step 6 - Compile, return, and write `.research.md`

Return the following structure. This is the required output format. Do not deviate.

After returning it, write the same content verbatim to `.research.md` in the project root. `/feature-design` reads `.brief.md` and `.research.md` automatically - the file is the pipeline, not the paste buffer.
...
/crisp-research   →  THIS SKILL: reads .crisp.md, writes .research.md
...
The output of `/crisp-research` is written to `.research.md`, which `/feature-design` reads automatically alongside `.brief.md`. The `PATTERNS FOUND`, `BENCHMARKS TO BEAT`, and `OPEN QUESTIONS FOR PM` sections map directly to the inputs `/feature-design` needs to produce a grounded feature spec.
