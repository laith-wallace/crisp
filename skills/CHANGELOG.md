...
- **P1** `/crisp-unslop` - the Slop Check for words. Edit mode makes the minimum effective edit to prose (landing copy, blog posts, docs, release notes, emails, PR bodies) and keeps the writer's voice via a three-line Voice Card that must survive verbatim; Detect mode names each pattern by id with the quoted line and a fix, and never scores or claims AI authorship. Nine pattern families (45 ids) in `references/patterns.md`, a 25-check binary eval in `references/eval.md`, and a grep-based mechanical pre-check (dash count, banned words with inflections, bold-label lines, recap openers) that runs before any reading for meaning. Adapted from cursor/plugins pstack `unslop` and petergyang `no-ai-slop` (both MIT), merged and deduped. UI strings stay with `/crisp-copy`.
- **P1** `/crisp-proof` - visual evidence for a design change. Before/after screenshot pairs across a state and breakpoint matrix (default, empty, loading, error, dense × 1280/375/768 × light/dark × focus-visible, expanded by what the change touched), `/crisp-review` grades on both sides, the detector on After, and a binary IMPROVED / NO CHANGE / REGRESSED verdict that leads the report. Six hard rules: never switch branches to manufacture a Before, never assume Before, never reuse a capture from another commit, never capture secrets, never full-page unless asked, never present a regression as a win. Capture path order: `@vercel/before-and-after` CLI → one-off Playwright → harness browser tools → ask. Workflow adapted from vercel-labs/before-and-after and michaelshimeles/skills evidence-driven-testing; no vendored code.
- **P1** `/crisp-loop` - bounded review-fix-review loop. Runs the detector then `/crisp-review`, fixes only the P0 and P1 issues the review cited with file:line, re-reviews, and stops on one of four named reasons: TARGET MET (default B with zero P0), CAP REACHED (default 5), STALLED (identical top 3 twice), or SCOPE BLOCKED (fix needs a file outside the declared scope list). Never lowers the target, never widens scope, never suppresses detector rules, never edits `.crisp.md` to move the register. `--commit` is opt-in; the build must pass after every fix. Loop shape adapted from greptileai `greploop` (MIT) with Greptile swapped for the pack's own reviewer.
- **P1** `/crisp-agents-md` - generates or refreshes a repo's `AGENTS.md`, the workflow file Codex, Cursor, Copilot, Gemini CLI, and Claude Code (via a `@AGENTS.md` include in `CLAUDE.md`) read at session start. Discovers the installed skill inventory and real repo facts (package manager, scripts, stack, CI, env var names from example files only) and maps them onto the six-beat CRISP workflow chain: Isolate (`/crisp-isolate`), Scope (`/crisp-brief`), Build (`/crisp-structure`), Prove (`/crisp-evidence`), Ship (`/crisp-proof` then `/crisp-loop`), Write (`/crisp-unslop`), with plain-rule fallbacks for any skill not installed and UI extras (`/feature-design`, `/crisp-copy`, `/crisp-a11y`, `/crisp-production-ready`) when the repo has a UI. Four modes: Create, Refresh (rewrites only the four managed blocks: workflow, commands, environment, skills), Adopt (appends managed blocks below an existing hand-written file), Check (drift report, zero writes). Binary rules verified by grep before delivery: zero em dashes, zero phantom slash commands, every listed command exists, env names only, under 200 lines, every gap a marked TODO. Structure inspired by michaelshimeles/skills `AGENTS.md`.
- **P1** `/crisp-structure` - two-layer code structure: actions own the why and when (business rules, auth, state transitions, failure classification), services own the how (explicit inputs, structured returns, zero domain-state writes). Three modes - Audit (findings with file:line for every duplicated mechanic, leaky service, fat component, and single-caller abstraction), Design (placement table for a new feature naming the exact service functions it reuses), Extract (eight-step one-caller-at-a-time migration). Twelve binary rules, a grep evidence pass before judgement, and a UI mapping (routes orchestrate, hooks and clients do, components render). Adapted from michaelshimeles/skills `code-structure`. Fixture: `tests/fixtures/duplicated-actions.ts`.
- **P1** `/crisp-evidence` - runtime proof a change works. The agent drives the app live while `scripts/evidence.sh` (bash, ffmpeg optional) records the screen and stamps `setup` / `test_start` / `assertion` annotations (passed, failed, untested; 80-char max) into the video, then writes `report.md` and `manifest.json` with the commit and branch. `doctor` reports toolchain and capture readiness; `--source none` keeps the annotation protocol and report on headless paths paired with numbered Playwright captures. Guardrails: never scripted playback as evidence, never secrets on screen, bug fixes capture the failure before the fix. Adapted from michaelshimeles/skills `evidence-driven-testing`; recorder rewritten from Python to bash.
- **P1** `/crisp-isolate` - one task, one worktree, one branch cut from `origin/main`. Scope check against open PRs (`gh pr diff --name-only`) and other agents' uncommitted work with a binary stop-and-ask on overlap, gitignored worktree directory verified with `git check-ignore`, fresh dependency install, port ownership check, and an Isolation receipt printed before any code. Harness deltas for Claude Code and Cursor, which manage worktrees themselves. Multi-agent rules: never `--force`, `--force-with-lease` only on your own branch, regenerate lockfiles. Adapted from michaelshimeles/skills `new-feature`.

### Router and tests
- **P2** `/crisp` router gains an **Isolate** row (`/crisp-isolate`), a **Build** row (`/crisp-structure`), a **Prove** row (`/crisp-evidence`), **Ship** rows for `/crisp-loop` and `/crisp-proof`, and a specialist row for `/crisp-unslop`. Together with `/crisp-agents-md` this completes the Isolate → Build → Prove → Ship agent workflow.
- **P2** New regression fixtures `tests/fixtures/duplicated-actions.ts` (for `/crisp-structure`, 7 findings and 3 must-not-flag traps) and `tests/fixtures/slop-prose.md` with `tests/expected/slop-prose.md` (19 pattern rows, 4 mechanical counts, 2 must-not-flag traps, Edit-mode additions).
